...
予約一覧は `user_account_id` / `therapist_account_id` / `therapist_profile_id` / `status` / `scheduled_from` / `scheduled_to` / `completed_on` / `q` / `sort` / `direction` で絞り込める。予約詳細は支払い状態、返金申請、通報、状態遷移ログ、施術場所情報を返し、詳細閲覧は `admin_audit_logs` に記録する。
...
問い合わせ一覧は `account_id` / `status` / `category` / `source` / `q` / `sort` / `direction` で絞り込める。詳細APIは送信者情報、本文、内部メモを返す。内部メモ追加と解決操作は `admin_notes` と `admin_audit_logs` に記録し、解決時は `status=resolved` と `resolved_at` を保存する。
