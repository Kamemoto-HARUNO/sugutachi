...
予約一覧は `user_account_id` / `therapist_account_id` / `therapist_profile_id` / `status` / `scheduled_from` / `scheduled_to` / `completed_on` / `q` / `sort` / `direction` で絞り込める。予約詳細は支払い状態、返金申請、通報、状態遷移ログ、施術場所情報を返し、詳細閲覧は `admin_audit_logs` に記録する。`GET /admin/bookings/{public_id}/messages` は `sender_account_id` / `moderation_status` / `detected_contact_exchange` / `read_status` で絞り込め、本文閲覧は `admin_audit_logs` に記録する。
