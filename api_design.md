...
料金ルール一覧は `account_id` / `therapist_profile_id` / `therapist_menu_id` / `rule_type` / `adjustment_bucket` / `adjustment_type` / `scope` / `is_active` / `q` / `sort` / `direction` で絞り込める。`adjustment_bucket` は `profile_adjustment` または `demand_fee` を受け付け、`scope` はプロフィール共通ルールかメニュー個別ルールかを示す。レスポンスにはセラピストアカウント、対象プロフィール、対象メニュー、`condition_summary` を含め、運営が不自然な設定を横断確認できるようにする。詳細APIは `condition_json`、価格上下限、優先度、適用対象を返し、閲覧時は `admin_audit_logs` に `pricing_rule.view` として記録する。
