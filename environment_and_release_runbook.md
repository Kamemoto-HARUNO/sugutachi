...
Xserver では PHP の絶対パスやプロジェクト配置先に合わせて、実際には次の形に寄せる想定です。

```cron
* * * * * /usr/bin/php /home/<account>/<project>/artisan schedule:run >> /home/<account>/<project>/storage/logs/schedule.log 2>&1
```
...
2. Xserver 上の staging / production の配置パスを確認する
3. cron を本番 / staging に設定する
4. GitHub Actions を使うかどうか決める
5. GTM の staging Environment 名と ID を控える
