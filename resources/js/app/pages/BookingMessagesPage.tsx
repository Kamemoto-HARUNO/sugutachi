...
export function BookingMessagesPage({role}: {role: 'user' | 'therapist'}) {
    ...
    useEffect(() => {
        ...
        void loadThreads();
        const refresh = () => { if (!document.hidden) void loadThreads(true); };
        const timer = window.setInterval(refresh, 30000);
        window.addEventListener('booking-message-summary:refresh', refresh);
        return () => {
            clearInterval(timer);
            window.removeEventListener('booking-message-summary:refresh', refresh);
        };
    ...
    const visibleThreads = threads.filter((booking) => buildCounterpartyName(booking).toLowerCase().includes(search.toLowerCase()));
    return (
        <section>
            <div className="space-y-2 px-4 pb-3">
                <input aria-label="会話を検索" type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="会話を検索" className="min-h-11 w-full rounded-full border border-slate-200 bg-slate-50 px-4 text-sm outline-none focus:border-[#b5894d]" />
                <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>予約ごとのやり取り</span>
                    <button type="button" onClick={() => void loadThreads(true)} disabled={isRefreshing} className="min-h-10 px-2 font-semibold">{isRefreshing ? '更新中…' : '更新'}</button>
                </div>
            </div>
            {error && <p role="alert" className="px-4 py-2 text-sm text-red-700">{error}</p>}
            {isLoading ? <p className="p-6 text-center text-sm text-slate-500">予約の連絡を読み込み中…</p> : visibleThreads.length === 0 ? (
                <p className="px-6 py-10 text-center text-sm text-slate-500">{search ? '一致する会話がありません。' : 'まだ予約の連絡はありません。'}</p>
            ) : (
                <div className="divide-y divide-slate-100">
                    {visibleThreads.map((booking) => {
                        const path = buildBookingMessagesDetailPath(role, booking.public_id);
                        const selected = location.pathname === path;
                        return <Link key={booking.public_id} to={path} aria-current={selected ? 'page' : undefined} className={`flex gap-3 border-l-2 px-4 py-4 transition ${selected ? 'border-[#b5894d] bg-[#faf5ee]' : 'border-transparent hover:bg-slate-50'}`}>
                            <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#f3e5d1] font-semibold text-[#6f4b26]">{buildCounterpartyAvatarLabel(booking)}</span>
                            <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2">
                                    <p className="truncate font-semibold">{buildCounterpartyName(booking)}</p>
                                    {booking.unread_message_count > 0 && <span className="shrink-0 text-xs font-semibold text-red-700">未読 {booking.unread_message_count}</span>}
                                </div>
                                <p className="mt-0.5 truncate text-xs text-slate-400">{buildScheduleLine(booking)}</p>
                                <p className="mt-1 truncate text-sm text-slate-600">{latestMessagePreview(booking, role)}</p>
                            </div>
                        </Link>;
                    })}
                </div>
