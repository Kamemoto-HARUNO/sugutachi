...
export function UserBookingReportPage() {
    ...
    usePageTitle(actorRole === 'therapist' ? '依頼者を通報する' : '通報する');
    ...
    const bookingDetailPath = publicId
        ? actorRole === 'therapist'
    ...
    const reportsPath = actorRole === 'therapist' ? bookingListPath : '/user/reports';
    const blocksPath = '/user/blocks';
    const showBlockList = actorRole === 'user';
    ...
    if (!booking || !targetAccountId) {
        return (
            <div className="space-y-6 rounded-[28px] bg-white p-6 text-[#17202b] shadow-[0_18px_36px_rgba(23,32,43,0.12)]">
                <p className="text-sm font-semibold text-[#9a7a49]">通報</p>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    >
                        {actorRole === 'therapist' ? '予約一覧へ' : '通報履歴へ'}
                    </Link>
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-8">
            <section className="rounded-[32px] bg-[linear-gradient(140deg,#17202b_0%,#223245_100%)] p-6 text-white shadow-[0_24px_60px_rgba(15,23,42,0.28)] sm:p-8">
                <p className="text-xs font-semibold tracking-wide text-[#f3dec0]">安全通報</p>
                <div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div className="space-y-3">
                        <h1 className="text-3xl font-semibold">予約に関する通報</h1>
                        <p className="max-w-3xl text-sm leading-7 text-slate-300">
                            安全上の不安や禁止行為の要求があったときは、ここから記録を残せます。送信後は運営側の確認に回り、必要に応じて後続対応が行われます。
                        </p>
                    </div>
                    <div className="rounded-[22px] border border-white/10 bg-white/5 px-5 py-4 text-sm text-slate-200">
                        ...
                        <p className="mt-2 text-lg font-semibold text-white">
                            {booking.counterparty?.display_name ?? (actorRole === 'user' ? booking.therapist_profile?.public_name : null) ?? '確認中'}
                        </p>
                    </div>
                </div>
            </section>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_380px]">
                <section className="space-y-6">
                    ...
                    <form onSubmit={(event) => void handleSubmit(event)} className="rounded-[28px] bg-white p-6 shadow-[0_18px_36px_rgba(23,32,43,0.12)]">
                        <p className="text-xs font-semibold tracking-wide text-[#9a7a49]">通報内容</p>
                ...
                <aside className="space-y-5">
                    <section className="rounded-[28px] bg-[#fffcf7] p-6 shadow-[0_18px_36px_rgba(23,32,43,0.1)]">
                        <p className="text-xs font-semibold tracking-wide text-[#9a7a49]">ブロック状態</p>
                    ...
                    {successReport ? (
                        <section className="rounded-[28px] bg-white p-6 shadow-[0_18px_36px_rgba(23,32,43,0.12)]">
                            <p className="text-xs font-semibold tracking-wide text-[#9a7a49]">次の確認先</p>
                            <p className="mt-3 text-sm leading-7 text-[#68707a]">
                                    {actorRole === 'therapist'
                                    ? '必要に応じて予約詳細や予約一覧から状況を確認してください。'
