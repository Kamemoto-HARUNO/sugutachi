...
function isTodayBooking(booking: BookingListRecord): boolean {
    if (!booking.scheduled_start_at) {
        return false;
    }
...
export function TherapistBookingsPage() {
    ...
    async function loadBookings(nextIsRefresh = false) {
        ...
        try {
            const payload = await apiRequest<ApiEnvelope<BookingListRecord[]>>('/bookings?role=therapist&sort=scheduled_start_at&direction=asc', {
                token,
            });
            ...
            <section className="rounded-[28px] bg-white p-5 shadow-[0_18px_36px_rgba(23,32,43,0.12)]">
                <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            <label htmlFor="therapist-request-type-filter" className="text-sm font-semibold text-[#17202b]">
                                予約タイプ
                            </label>
                            <select
                                id="therapist-request-type-filter"
                                value={requestType}
                                onChange={(event) => {
                                    setSearchParams((previous) => {
                                        const next = new URLSearchParams(previous);
                        ...
                        <div className="space-y-2">
                            <label htmlFor="therapist-sort-mode" className="text-sm font-semibold text-[#17202b]">
                                並び順
                            </label>
                            <select
                                id="therapist-sort-mode"
                                value={sortMode}
                                onChange={(event) => {
                                    setSearchParams((previous) => {
                                        const next = new URLSearchParams(previous);
            ...
            {filteredBookings.length > 0 ? (
                <section className="grid gap-4">
                    {filteredBookings.map((booking) => {
                        ...
                        const displayName = booking.counterparty?.display_name ?? '利用者を確認中';
                                ...
                                <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                                            ...
                                            <div>
                                                <p className="text-xs font-semibold tracking-wide text-[#9a7a49]">受取予定額</p>
                                                <p className="mt-2 text-sm font-semibold text-[#17202b]">
