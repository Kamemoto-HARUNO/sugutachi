...
export function AccountProfilePage() {
    ...
    const loadProfile = useCallback(async () => {
        ...
        setPhoneNumber(nextProfile.phone_e164 ?? '');
    }, [token]);
        ...
        try {
            const payload = await apiRequest<ApiEnvelope<MeProfileRecord>>('/me/profile', {
                ...
                body: {
                    ...
                    phone_e164: phoneNumber.trim() || null,
                },
            });
            ...
            const message = requestError instanceof ApiError ? requestError.message : 'アカウント情報の更新に失敗しました。';
            setError(message);
    ...
    if (isLoading) {
        return <LoadingScreen title="アカウント情報を確認中" message="共通で使うプロフィール情報を読み込んでいます。" />;
    }
        ...
        <div className="mx-auto w-full max-w-[1180px] space-y-10 px-4 py-8 sm:px-6 lg:px-8">
            <section className="grid gap-10 lg:grid-cols-[minmax(0,1.3fr)_360px] lg:items-start">
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="max-w-3xl text-sm leading-7 text-slate-300 sm:text-[0.95rem]">
                                ここで変更した表示名、電話番号、本人確認の状態は、利用者とセラピストのどちらでも同じ内容が使われます。
                    ...
                    <form onSubmit={handleSubmit} className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-[0_24px_60px_rgba(2,6,23,0.18)]">
                        <div className="space-y-2">
                            ...
                            <p className="text-sm leading-7 text-slate-300">
                                メールアドレスはログイン用として固定です。表示名と電話番号はここから更新できます。
                            </p>
                        </div>
                        ...
                        <div className="mt-6 grid gap-4 md:grid-cols-2">
                            <label className="space-y-2">
                                ...
                                <input
                                    ...
                                    placeholder="山田"
                                />
                                {getFieldError(requestError, 'display_name') ? (
                                    <p className="text-xs text-amber-200">{getFieldError(requestError, 'display_name')}</p>
                                ) : null}
                            </label>
                            ...
                            <label className="space-y-2">
                                ...
                                />
                                <p className="text-xs text-slate-400">国番号から入力してください。例: +819012345678</p>
                                {getFieldError(requestError, 'phone_e164') ? (
                                    <p className="text-xs text-amber-200">{getFieldError(requestError, 'phone_e164')}</p>
                                ) : null}
                            </label>
                            ...
                            <label className="space-y-2 md:col-span-2">
                                ...
                                />
                                <p className="text-xs text-slate-400">メールアドレスの変更は現在この画面では行えません。</p>
                            </label>
                        </div>
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        <div className="rounded-[20px] border border-white/10 bg-[#111923] px-4 py-4">
                            ...
                            <div className="mt-3 flex items-center gap-3">
                                ...
                                <p className="text-sm text-slate-300">{profile?.phone_e164 ?? '未設定'}</p>
                            </div>
                        </div>
